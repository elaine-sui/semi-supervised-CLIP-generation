...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        prefix = self.images[img_id]["embed"]
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2",
                 normalize_prefix=False, seed=1234):
        ...
        random.Random(seed).shuffle(self.cap_ids)
...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix) in enumerate(train_dataloader):
            ...
            progress.update()
            losses.update(loss.item(), logits.shape[0])
            if (idx + 1) % 10000 == 0:
                torch.save(
                    model.state_dict(),
        ...
        progress.close()
        lr = optimizer.param_groups[0]['lr']
        wandb.log({"train_loss_epoch": losses.avg, "epoch": epoch, "lr": lr})
...
def main():
    ...
    parser.add_argument('--name', default='og_clip_prefix_cap_coco')
    parser.add_argument('--project', default='clip_prefix_cap_coco')
    parser.add_argument('--entity', default='elainesui')
    ...
    experiment_name = set_up_wandb(args)
    args.out_dir = f"{args.out_dir}/{experiment_name}"
